...
namespace QLCafe.DAO
{
    public class DAO_KetCa
    {
        ...
        {
            string sTruyVan = string.Format(@"SELECT GioVao FROM [CF_HoaDon] WHERE IDNhanVien = {0} AND TrangThai = 1 AND TrangThaiKetCa = 0", IDNhanVien);
            DataTable data = new DataTable();
            data = DataProvider.TruyVanLayDuLieu(sTruyVan);
            if (data.Rows.Count > 0)
            {
                DataRow dr = data.Rows[0];
                return dr["GioVao"].ToString();
            }
            else
        ...
        {
            string sTruyVan = string.Format(@"SELECT GioRa FROM [CF_HoaDon] WHERE IDNhanVien = {0} AND TrangThai = 1 AND TrangThaiKetCa = 0", IDNhanVien);
            DataTable data = new DataTable();
            data = DataProvider.TruyVanLayDuLieu(sTruyVan);
            if (data.Rows.Count > 0)
            {
        ...
        public static bool ThemKetCa(int IDNhanVien, string GioVao, string GioRa, double TongTienSauCa, float tongGio, string IDChiNhanh, double TongTien, double GiamGia)
        {
            string sTruyVan = string.Format(@"INSERT INTO CF_KetCa(ThoiGianBatDau,ThoiGianKetThuc,TongTienSauCa,IDNhanVien,IDChiNhanh,TongTien,GiamGia,TongThoiGian) VALUES ('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}')", DateTime.Parse(GioVao).ToString("yyyy-MM-dd hh:mm:ss tt"), DateTime.Parse(GioRa).ToString("yyyy-MM-dd hh:mm:ss tt"), TongTienSauCa, IDNhanVien, IDChiNhanh, TongTien, GiamGia, tongGio);
            return DataProvider.TruyVanKhongLayDuLieu(sTruyVan);
        }
    }
}
